...
inline static struct sockaddr_in get_address(char *host, uint16_t port) {
  ...
  address.sin_addr.s_addr = ((struct sockaddr_in *)(address_result->ai_addr))
...
inline static void connect_socket_ip6(int socket_fd,
...
inline static void send_message_address(int socket_fd, const void *message,
                                        size_t length, int flags,
                                        struct sockaddr *address,
                                        socklen_t addr_len) {
  errno = 0;
  ssize_t sent_length =
      sendto(socket_fd, message, length, flags, address, addr_len);
  if (sent_length < 0) {
    printf("Failed sending %.*s\n", (int)sent_length, (char *)message);
    PRINT_ERRNO();
  }
  ENSURE(sent_length == (ssize_t)length);
}
...
inline static size_t receive_message_address(int socket_fd, void *buffer,
                                             size_t max_length, int flags,
                                             struct sockaddr *address,
                                             socklen_t *addr_len) {
  errno = 0;
