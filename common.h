...
inline static void connect_socket_ip6(int socket_fd, const struct sockaddr_in6 *address) {
    CHECK_ERRNO(connect(socket_fd, (struct sockaddr *) address, sizeof(*address)));
}
...
inline static size_t receive_message_address(int socket_fd, void *buffer, size_t max_length, int flags, struct sockaddr *address, socklen_t *addr_len) {
  errno = 0;
  ssize_t received_length = recvfrom(socket_fd, buffer, max_length, flags, address, addr_len);
  if (received_length < 0) {
    PRINT_ERRNO();
  }
  return (size_t) received_length;
